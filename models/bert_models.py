...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        elif len(query.size()) == 4:
            s2, s3 = query.size(1), query.size(2)
            query, key, value = [l(x).view(batch_size, s2, s3, self.h, self.d_k).transpose(-2, -3)
                                for l, x in zip(self.linear_layers, (query, key, value))]
        ...
        elif len(query.size()) == 5:
            x = x.transpose(-2, -3).contiguous().view(
                batch_size, s2, s3, self.h * self.d_k)
...
class PreTrainedBertModel(nn.Module):
    ...
    @classmethod
    def from_pretrained(cls, pretrained_model_name, state_dict=None, cache_dir='', logger=None, *inputs, **kwargs):
